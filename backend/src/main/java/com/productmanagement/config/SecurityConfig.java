...
public class SecurityConfig {
    ...
    @Value("${app.security.permit-all-products:false}")
    private boolean permitAllProducts;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authz -> {
                    ...
                    authz.requestMatchers("/swagger-ui.html", "/swagger-ui/**", "/v3/api-docs/**", "/actuator/**").permitAll();
                    authz.requestMatchers("/uploads/**").permitAll();
                    authz.requestMatchers(HttpMethod.GET, "/api/products/**").permitAll();
                    if (permitAllProducts) {
